...
### 2. Owner/admin Google OAuth — `cmd-auth.mjs` → `~/.if/creds/.env.auth.<email>.json`
...
**Cred storage:** all OAuth creds live at `~/.if/creds/.env.auth.<email>.json` — out-of-tree, one file per Google account, chmod 600. Never in the project. There is no "default" file; the account always comes from either a positional arg or `EMAIL_OF_GOOGLE_HOSTING_ACCOUNT` in `.env`.
...
**Default account:** `cmd-auth.mjs` (and the deploy wrapper) reads `EMAIL_OF_GOOGLE_HOSTING_ACCOUNT` from `.env` to know which account this checkout uses. Set it in `.env` alongside `THIS_PROJECT_ID_ON_GOOGLE_HOSTING`.

**Multi-account:** pass the email as a positional arg — `node cmd-auth.mjs alice@x.com --token` — uses `~/.if/creds/.env.auth.alice@x.com.json`. Add a new account with `npm run auth -- alice@x.com`. Probe-only check: `npm run auth:status` (or `node cmd-auth.mjs alice@x.com --status`).
...
The wrapper (`cmd-deploy.mjs`) reads the OAuth refresh token from `~/.if/creds/.env.auth.<EMAIL_OF_GOOGLE_HOSTING_ACCOUNT>.json`, materialises a temporary `.adc.json`, points `GOOGLE_APPLICATION_CREDENTIALS` at it, and runs `firebase deploy` for you. **You don't need to fetch a token first** — the wrapper does it. Don't pre-call `cmd-auth.mjs --token` before deploying; it's pointless and can mask issues.
...
**Multi-account deploy:** `EMAIL_OF_GOOGLE_HOSTING_ACCOUNT=alice@x.com npm run deploy:functions` picks up `~/.if/creds/.env.auth.alice@x.com.json` for that one run.

**`.env`** carries `THIS_PROJECT_ID_ON_GOOGLE_HOSTING`, `THIS_PROJECT_REGION_ON_GOOGLE_HOSTING`, and `EMAIL_OF_GOOGLE_HOSTING_ACCOUNT` (gitignored, 1:1 with checkout — no dev/test/prod split for this template's audience). If missing, write them manually. The region is the immutable Firestore location; the functions build bakes it into `functions/src/region.ts` (see CLAUDE-STACK.md → Region). `EMAIL_OF_GOOGLE_HOSTING_ACCOUNT` picks the cred file at `~/.if/creds/.env.auth.<email>.json` for both `cmd-auth.mjs` (when no positional arg) and `cmd-deploy.mjs`.
