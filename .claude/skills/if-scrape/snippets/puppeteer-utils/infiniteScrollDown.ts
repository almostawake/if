...
export async function infiniteScrollDown({
    ...
    debug && console.log(`[${runId}] Starting infinite scroll down for ${times} times for selector: ${loadableItemSelector}`);
    ...
    do {
        ...
        if (scrollAttempts >= times) {
            debug && console.log(`[${runId}] Reached maximum scroll times (${times}), stopping infinite scroll`);
            break;
        }
